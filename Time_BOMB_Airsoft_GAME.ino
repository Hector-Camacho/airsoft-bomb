 ...
 */
char* menu1[]={"Buscar y destruir","Sabotaje","Dominacion", "Configuracion"      };
char* menu2[]={"Configuracion de juego","Configuracuin de sonido", "Prueba relay", "Prueba Test"      };   
char* GAME_TIME="Tiempo de juego:";
char* BOMB_TIME="Tiempo de detonacion:";
...
char* ENABLE_RELAYPIN="Activar Relay?";
char* ENABLE_CODE="Activar codig?";
char* GAME_TIME_TOP="TIEMPO DE JUEGO";
char* ARMING_BOMB = "ARMANDO BOMBA";
char* ENTER_CODE = "Introduce el codigo";
char* CODE_ERROR = "Error de codigo!";
char* BOMB_ARMED = "BOMBA ARMADA";
char* DETONATION_IN = "DETONACION EN";
char* DISARMING = "DESARMANDO BOMBA" ;
char* DISARM = "DESARMANDO...";
char* GAME_OVER = " FIN DEL JUEGO! ";
char* DEFENDERS_WIN = " DEFENSORES GANAN ";
char* SABOTAGE_FAIL= "FALLO EL SABOTAJE!";
...
void setup(){
  ...
  lcd.print("F.O.R.T AIRSOFT");// you can add your team name or someting cool
  lcd.setCursor(0,1);
  lcd.print("BOMBA AIRSOFT");// you can add your team name or someting cool
...
void domination(){
  ...
  while(1)  // this is the important code, is a little messy but works good.
  {
    ...
    if(showGameTime){ //THE SECOND IS /2
      ...
      if(team == 1)lcd.print(" ZONA AMARILLA");
...
void gameOver(){
  ...
  while(!defusing){
    ...
    if(greenTime>redTime){
      //greenteam wins
      lcd.print(" EQUIPO AMARILLO GANA!");
      digitalWrite(GREENLED, HIGH);
    }
    else{
      //redteam wins 
      lcd.print("  EQUIPO ROJO GANA!");
      digitalWrite(REDLED, HIGH);
    }
    delay(3000);
    keypad.getKey();
...
void setNewPass(){
  ...
  while(1){
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Escribir contraseña");
    setPass();

    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Repetir contraseña");
    ...
    if(comparePassword()){
      ...
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("ERROR No son iguales!");
